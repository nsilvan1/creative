...
AddEventHandler("cemitery:initBody", function()
	...
	if not Peds then
		...
		if LoadModel(List[Hash]) then
			Peds = CreatePed(4, List[Hash], Locates[Selected][1], Locates[Selected][2], Locates[Selected][3] - 1,
				Locates[Selected][4] - 180.0, false, false)
			...
			exports["target"]:AddCircleZone("Cemitery:" .. Selected,
				vec3(Locates[Selected][1], Locates[Selected][2], Locates[Selected][3]), 0.5, {
			...
			TriggerEvent("Notify", "default",
				"Parece que estão efetuando uma limpeza em um dos túmulos, guarde essa informação e veja se você encontra alguns objetos de valor.",
				"Observação", 10000)
		end
	end
end)
