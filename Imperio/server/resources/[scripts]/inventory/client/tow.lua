...
-----------------------------------------------------------------------------------------------------------------------------------------
local Vehicles = {
	"blista", "dilettante", "issi2", "panto", "prairie", "rhapsody", "felon", "oracle", "buccaneer", "chino", "picador",
	"sabregt", "virgo", "bfinjection", "rancherxl", "baller", "bjxl", "cavalcade", "gresley", "habanero", "landstalker",
	"radi", "rocoto", "seminole", "serrano", "asea", "asterope", "emperor", "ingot", "intruder", "premier", "primo",
	"regina", "stanier", "stratum"
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- RESCUES
-----------------------------------------------------------------------------------------------------------------------------------------
	...
	if GetGameTimer() >= Time then
		...
		if Active then
			...
			TriggerEvent("NotifyPush",
				{ code = 20, title = "Registro de Veículo", x = Rescues[Selected]["x"], y = Rescues[Selected]["y"],
					z = Rescues[Selected]["z"], vehicle = VehicleName(Vehicles[Model]), color = 2 })
		end
	...
	while true do
		if Active and not Spawned then
			...
			if Distance <= 100 then
				Spawned = true
				vGARAGE.ServerVehicle(Vehicles[Model], Rescues[Selected]["x"], Rescues[Selected]["y"],
					Rescues[Selected]["z"], Rescues[Selected]["w"], nil, 0, nil, 100, 0)
			end
		end
...
AddEventHandler("inventory:Tow", function(Entity)
	if Entity[2] == Vehicles[Model] then
		...
		TriggerEvent("NotifyPush",
			{ code = 20, title = "Registro de Veículo", x = Rescues[Selected]["x"], y = Rescues[Selected]["y"],
				z = Rescues[Selected]["z"], name = "Aguardando reboque", color = 2 })
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- INVENTORY:INVOKETOW
	...
	if NetworkDoesNetworkIdExist(Vehicle) and NetworkDoesNetworkIdExist(Vehicle02) then
		...
		if DoesEntityExist(Vehicle) and DoesEntityExist(Vehicle02) then
			if Mode == "in" then
				...
				AttachEntityToEntity(Vehicle02, Vehicle, GetEntityBoneIndexByName(Vehicle, "bodyshell"), 0, -2.2,
					0.4 - Min["z"], 0, 0, 0, true, true, false, true, 2, true)
